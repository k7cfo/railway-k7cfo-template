...
Generated apps include a single `pnpm verify` quality gate for formatting, linting, strict types, dead-code detection, unit/API/browser tests, the production build, and production-server smoke testing. Their GitHub Actions workflow can block Railway deployment until those checks pass. This template repository also generates a clean app and repeats the PostgreSQL, browser, production, and Docker checks on every pull request.
