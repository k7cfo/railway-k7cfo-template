...
export function AuthShell({ appTitle, title, description, children, footer }: AuthShellProps) {
  return (
      ...
      <main className="relative z-10 flex flex-1 items-center justify-center px-6 pb-20 sm:px-14">
        <div className="w-full max-w-md">
          <div className="mb-6">
            ...
            {description && <p className="m-0 text-sm leading-relaxed text-[var(--fg-2)]">{description}</p>}
          </div>

          <div className="border-2 border-[var(--border-1)] bg-[var(--bg-1)] p-6">{children}</div>

          {footer && <div className="mt-6 text-center text-sm text-[var(--fg-3)]">{footer}</div>}
        </div>
      </main>
    </div>
  );
