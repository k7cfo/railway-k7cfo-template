            ...
            <Reveal key={plan.name} delay={i * 0.1} className="flex">
              <article
                className={
                  ...
                  (plan.featured ? "border-[var(--accent)] border-t-4" : "border-[var(--border-1)]")
                }
              >
                {/* Header row */}
                ...
                <div className="px-5 pt-6 pb-4">
                  <div className="font-mono text-[48px] leading-none font-medium tracking-[-0.02em] text-[var(--fg-0)]">
                    {plan.price}
                    {plan.cadence && (
                      <span className="text-[14px] tracking-[0] text-[var(--fg-3)]">{plan.cadence}</span>
                    )}
                  </div>
                  {plan.description && (
                  ...
                  {plan.features.map((feature) => (
                    ...
                    >
                      <span
                        aria-hidden="true"
                        className={
                          "text-[10px] " + (plan.featured ? "text-[var(--accent)]" : "text-[var(--fg-3)]")
                        }
                      >
                        ▪
                      </span>
                ...
                <div className="mt-auto border-t border-[var(--border-1)] p-5">
                  <Button size="lg" variant={plan.featured ? "accent" : "outline"} className="w-full" asChild>
                    <Link to={plan.cta.to}>
