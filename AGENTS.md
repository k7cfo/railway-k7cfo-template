...
After changes, generate a disposable project with Copier and verify installation, setup, migration, seed, `pnpm verify`, non-root Docker build/runtime, and tracked-secret checks. Treat failed formatting, lint, strict types, dead-code detection, tests, production smoke checks, or Docker checks as blockers; do not weaken a quality rule to force a pass. For Tailscale changes, test the pure configuration behavior and missing-CLI failure path; run the live `pnpm tailscale:check` only when the environment is connected to a tailnet, and report when it is unavailable. Never commit a generated smoke app, real secret, or environment file. Preserve `.copier-answers.yml`, the generated marker, and the generated `CLAUDE.md → AGENTS.md` symlink.
